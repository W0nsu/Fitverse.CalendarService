...
namespace Fitverse.CalendarService.MessageBus.Recivers
{
	public class DeleteMemberReciver : BackgroundService
	{
		...
		private void InitializeRabbitMqListener()
		{
			...
			_channel.QueueDeclare("CS_DeleteMember", false, false, false, null);
			_channel.QueueBind("CS_DeleteMember", "members", "deleteMember", null);
		}
		...
		protected override Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			consumer.Received += (ch, ea) =>
			{
				...
				var deletedMemberReservations = dbContext.Reservations
					.Where(x => x.MemberId == deletedMemberEntity.MemberId)
					.ToList();

				foreach (var reservation in deletedMemberReservations)
					_ = dbContext.Remove(reservation);
			...
			_channel.BasicConsume("CS_DeleteMember", false, consumer);
